...
// 与当前网页使用同一主机和端口，兼容任意局域网网段及HTTPS反向代理。
...
websocket.subscribe('message', (data) => {
    try {
...
function updateMachinePosition(map) { // 接收地图对象作为参数
    // 从 localStorage 获取positionData JSON对象
    const positionDataStr = localStorage.getItem('positionData');
    let lat, lng, orientationW, orientationX, orientationY, orientationZ;
    
    if (positionDataStr) {
        try {
            const positionData = JSON.parse(positionDataStr);
            lat = parseFloat(positionData.latitudeCurrent);
            lng = parseFloat(positionData.longitudeCurrent);
            orientationW = parseFloat(positionData.orientationW) || 0;
            orientationX = parseFloat(positionData.orientationX) || 0;
            orientationY = parseFloat(positionData.orientationY) || 0;
            orientationZ = parseFloat(positionData.orientationZ) || 0;
        } catch (e) {
    ...
    // 再次确保coords对象及其属性不为null
    if (!coords || coords.lat === null || coords.lng === null) {
        console.warn('坐标对象或其属性为null，跳过更新');
        return;
    }
...
function createMachineMarker(coords, map, rotationDegrees = 0) { // 接收地图对象和旋转角度作为参数
    ...
    // 验证坐标对象
    if (!coords || coords.lat === null || coords.lng === null || isNaN(coords.lat) || isNaN(coords.lng)) {
        console.error('无法创建标记：坐标对象无效');
        return null;
    }

    const machineIcon = L.divIcon({
        className: 'machine-marker',
        html: `<img src="../../../src/assets/imges/车.svg" style="width: 30px; height: 30px; transform: rotate(${rotationDegrees}deg);" />`,
        iconSize: [30, 30],
        iconAnchor: [15, 15]
    });
...
function updateMachineMarkerRotation(rotationDegrees) {
    ...
    try {
        ...
        const newIcon = L.divIcon({
            className: 'machine-marker',
            html: `<img src="../../../src/assets/imges/车.svg" style="width: 30px; height: 30px; transform: rotate(${rotationDegrees}deg); transition: transform 0.2s ease-in-out;" />`,
            iconSize: [30, 30],
            iconAnchor: [15, 15]
        });
...
function clearScreenAndReset() {
   ...
   if (toggleStatus) {
       ...
       if (addEdit) {
           addEdit.src = '../../../src/assets/imges/img-right/绘制路线 (1).png';
       }
   }
...
async function onLoad() {
    ...
    map = L.map('mapDiv', {
        ...
        zoomSnap: 0.1
    }).setView([initlatitudeCurrent, initlongitudeCurrent], 23);

    // 手机端适配：触屏滑动时桥接为 mousemove，保证画线预览/坐标跟随等功能可用
    (function bridgeTouchToMouse(m) {
        const container = m.getContainer();
        if (!container || !('ontouchstart' in window)) return;
        container.addEventListener('touchmove', function (e) {
            // 仅当页面注册了 mousemove 监听时才转发，避免拖动地图时误触发
            if (!m.listens('mousemove')) return;
            const touch = e.touches[0];
            if (!touch) return;
            const rect = container.getBoundingClientRect();
            const latlng = m.containerPointToLatLng(
                L.point(touch.clientX - rect.left, touch.clientY - rect.top)
            );
            m.fire('mousemove', { latlng: latlng, originalEvent: e });
        }, { passive: false });
    })(map);
   ...
   // 原地图点击隐藏逻辑函数
    ...
    // 添加图标
    const icon = L.icon({
        iconUrl: '../../../src/assets/imges/当前位置 (2).png', // 替换为你的图标URL
        iconSize: [16, 16], // 图标大小
        iconAnchor: [8, 8] // 图标锚点
    });
    ...
    var customIcon = L.icon({
        iconUrl: '../../../src/assets/imges/坐标.svg',
        iconSize: [22, 22],
        iconAnchor: [11, 11]
    });
    map.on('click', () => {
...
function toggleStationVisibility() {
    ...
    if (stationVisibility) {
        showHideImg.src = '../../../src/assets/imges/img-right/显示隐藏.png';
    } else {
        showHideImg.src = '../../../src/assets/imges/img-right/闭眼.png';
    }
    
    // 创建一个包含所有可能标记的数组
    const allMarkers = [...markers];
    
    // 如果存在editableMarkers数组，也将其包含进来
    if (typeof editableMarkers !== 'undefined' && Array.isArray(editableMarkers)) {
        allMarkers.push(...editableMarkers);
    }
    ...
    allMarkers.forEach(marker => {
        try {
            ...
            // 检查标记是否为特殊站点标记 - 通过多种方式判断
            let isSpecialStationMarker = false;
            
            // 方法1：通过图标URL判断
            if (marker.options.icon && 
                marker.options.icon.options && 
                marker.options.icon.options.iconUrl &&
                (marker.options.icon.options.iconUrl.includes('坐标.png') || 
                 marker.options.icon.options.iconUrl.includes('坐标.svg'))) {
                isSpecialStationMarker = true;
            }
            
            // 方法2：通过坐标匹配判断
            if (!isSpecialStationMarker) {
                const markerPos = marker.getLatLng();
                isSpecialStationMarker = stationData.some(station => 
                    Math.abs(markerPos.lat - station.latitude) < 1e-6 && 
                    Math.abs(markerPos.lng - station.longitude) < 1e-6
                );
            }
            ...
            if (isSpecialStationMarker) {
                ...
                const station = stationData.find(s => 
                    Math.abs(markerPos.lat - s.latitude) < 1e-6 && 
                    Math.abs(markerPos.lng - s.longitude) < 1e-6
                );
                
                if (station) {
                    if (stationVisibility) {
                        // 显示标签 - 重新绑定tooltip，设置为永久显示
                        ...
                        marker.bindTooltip(station.stationName, {
                            ...
                            direction: 'right',
                            offset: [20, 0],
                            permanent: true  // 设置为永久显示
                        }).openTooltip();  // 立即打开tooltip
                    } else {
                        // 隐藏标签 - 解绑tooltip
                        if (marker.getTooltip()) {
                            marker.unbindTooltip();
...
function showAddButtonNearStation(stationMarker, routeId, stationIndex) {
  ...
  const addButtonIcon = L.icon({
    iconUrl: '../../../src/assets/imges/img-right/add.png', // 使用正确的加号图标路径
    iconSize: [24, 24],
    iconAnchor: [12, 12]
  });
  ...
  addButtonMarker.on('click', function(e) {
    ...
    // var customIcon = L.icon({
    //     iconUrl: '../../../src/assets/imges/原点.png',
    //     iconSize: [22, 22],
    //     iconAnchor: [11, 11]
    // });
...
function showAddButtonNearRoutePoint(point, routeMarker) {
    ...
    const addButtonIcon = L.icon({
        iconUrl: '../../../src/assets/imges/img-right/add.png', // 使用与showAddButtonNearStation函数相同的图标路径
        iconSize: [24, 24],
...
`;
    ...
    // 先添加到路线中，这会调用recreateDistanceAndRectMarkers()函数创建矩形框
    addToRoute(latLng);
    
    // 然后再重置编辑状态，确保矩形框不会被清除
    // disableEditing();
    // enableEditing();
}
...
const testPosition = () => {
    ...
    if (positionDataStr) {
        try {
            const positionData = JSON.parse(positionDataStr);
            latitudeCurrent = parseFloat(positionData.latitudeCurrent);
            longitudeCurrent = parseFloat(positionData.longitudeCurrent);
        } catch (e) {
            console.error("解析positionData失败:", e);
...
function getCurrentCarPosi() {
    ...
    if (longitudeCurrent == '未运行' || latitudeCurrent == '未运行' || latitudeCurrent == '0' || latitudeCurrent == "NaN") {
        ...
        map.setView([Number(latitudeCurrent), Number(longitudeCurrent)], 25);
    }
}
...
function recreateAllPointMarkers() {
    ...
    routePoints.forEach((point, index) => {
        ...
        if (stopValue === "0" || stopValue === 0) {
            ...
            // 否则使用任务进程.svg图标
            routeIcon = L.icon({
                iconUrl: '../../../src/assets/imges/任务进程.svg',
                iconSize: [25, 25],
                iconAnchor: [12.5, 25]
            });
        }
    ...
    if (isSpecialStationLocation) {
        ...
        if (specialStation) {
            ...
            const routeIcon = L.icon({
                iconUrl: '../../../src/assets/imges/坐标.png',
                iconSize: [22, 22],
                iconAnchor: [11, 11]
            });
...
async function editToggleClickEventWithRoute() {
    if (!editRouteStatus) {
        ...
        imgEdit.src = '../../../src/assets/imges/img-right/编辑状态.png';
        editStatus = false
        ...
        // addEdit.src = '../../../src/assets/imges/img-right/绘制路线 (2).png';
            ...
            if (selectedRoute && selectedRoute.stations && selectedRoute.stations.length > 0) {
                ...
                if (savedTempRouteStations && savedTempRouteStations.length > 0) {
                    savedTempRouteStations.forEach(tempStation => {
                        ...
                        if (originalStation) {
                            ...
                            originalStation.stopTime = tempStation.stopTime;
                ...
                savedRouteData.stations.forEach(station => {
                    ...
                    const stationDataToSave = updatedStation || {
                        ...
                        stop: station.stop || '0',
                        stopTime: station.stopTime != null ? station.stopTime : '0',
                    };
        ...
        let imgEdit = document.getElementById("imgEdit");
        document.querySelector(".text-edit").textContent = "开始编辑";
        imgEdit.src = '../../../src/assets/imges/img-right/编辑.png';
        document.querySelector(".text-editLine").textContent = "新增路线";
...
// 定义打印当前路线ID的函数，这里主要是发布话题
        async function pubTask() {
            ...
            try {
                ...
                cocoMessage.success("暂停成功");
                }
                return response; // 返回响应数据，方便上层函数处理
            } catch (error) {
                console.error(`操作失败:`, error);
                // 可以根据需要添加错误处理逻辑，如重试机制或用户提示
                return null;
            }
        }
...
function loadRoute(routeId) {
    ...
    if (window.isAddingToExistingRoute) {
        ...
        const routePoints = selectedRoute.stations.map(station => {
            ...
            marker.on('mouseout', function() {
                // 不再关闭tooltip，因为permanent: true会保持标签始终可见
                // 移除显示类
                const tooltip = this.getTooltip();
                if (tooltip) {
                    L.DomUtil.removeClass(tooltip._container, 'leaflet-tooltip-visible');
...
function showMultiplePointInfoBoxes(event, stations, startIndex = 0) {
    ...
    if (stations.length > 0) {
        ...
        const selectedIcon = L.icon({
            iconUrl: '../../../src/assets/imges/坐标 (3).svg', // 使用一个特殊的图标来表示选中的站点
...
async function savePointInfo(stationId, boxIndex, station) {
    ...
    const workDurationInput = document.getElementById(`stopTime-${stationId}-${boxIndex}`);
    ...
    if (!speedInput || !workDurationInput || !obstacleSelect || !directionInput || !taskSelect || !positionSelect || !lanechangeSelect || !stopRadios || !runmodeRadios) {
        return;
    }

    const rawWorkDuration = workDurationInput.value.trim();
    const parsedWorkDuration = rawWorkDuration === '' ? 0 : Number(rawWorkDuration);
    ...
    if (!stationId || stationId === 'null' || stationId === null || station.isRoutePoint || window.isAddingToExistingRoute || window.isNewRouteMode || (window.tempRouteStations && window.tempRouteStations.length > 0)) {
        ...
        const tempStationData = {
            ...
            stopTime: normalizedWorkDuration,
        ...
        if (markerIndex > -1 && markers[markerIndex]) {
            ...
            if (isSpecialStationMarker) {
                ...
                if (isStopStation && !isFirstStation && !isLastStation) {
                    ...
                    // 起点使用起点图标
                    const startIcon = L.icon({
                        iconUrl: '../../../src/assets/imges/英文起点.png',
                        iconSize: [25, 25],
                        iconAnchor: [10, 18]
                    });
                    ...
                    const endIcon = L.icon({
                        iconUrl: '../../../src/assets/imges/英文终点.png',
                        iconSize: [25, 25],
                        iconAnchor: [10, 18]
                    });
                    ...
                    if (stopValue === "0" || stopValue === 0) {
                        ...
                        const circleFillIcon = L.icon({
                            iconUrl: '../../../src/assets/imges/circle-fill.svg',
                            iconSize: [12, 12],
                            iconAnchor: [6, 6]
                        });
    ...
    if (selectedRoute) {
        ...
        if (station) {
            ...
            station.stopTime = normalizedWorkDuration;
...
function createSinglePointInfoBox(station, index, boxIndex) {
    ...
    const stationStopTime = station.stopTime != null && String(station.stopTime).trim() !== ''
        ? String(station.stopTime)
        : '0';
...
<div>作业时长:<input type="number" value='${stationStopTime}' id="stopTime-${station.stationId}-${boxIndex}" class="param-input" placeholder="作业时长(秒)" min="0" step="1" inputmode="decimal" aria-label="作业时长，单位秒" style="width: 160px;" ${disableInputs ? 'disabled' : ''}><span style="margin-left: 5px; font-size: 12px;">秒</span></div>
...
<div style="margin: 5px 0;">
  ...
  <input type="number" 
         ...
         min="0" 
         step="0.1" 
         value="${station.avoidDistance || localStorage.getItem('globalAvoidDistance') || 0.5}" 
         ${disableInputs ? 'disabled' : ''}>
        ...
        if (selectedRoute) {
            ...
            if (stationDataItem) {
                ...
                const newStation = {
                    ...
                    stopTime: stationDataItem && stationDataItem.stopTime != null ? stationDataItem.stopTime : "0",
                    ...
                    avoidDistance: tempStation ? (tempStation.avoidDistance || "0.5") : (originalStation ? (originalStation.avoidDistance || "0.5") : (localStorage.getItem('globalAvoidDistance') || "0.5"))
                };
                selectedRoute.stations.push(newStation);
            }
...
async function populateRouteSelect(selectLast = false, selectId) {
    ...
    selectElement.addEventListener('change', function (e) {
        ...
        if (e.target.value) {
            ...
            if (selectedRoute) {
                ...
                selectedRoute.stations.forEach((station, index) => {
                    ...
                    const stopIcon = L.icon({
                        iconUrl: '../../../src/assets/imges/任务进程.svg', // 停车站点图标，用户可以根据需要修改
                        iconSize: [25, 25],
                        iconAnchor: [12.5, 25]
                    });
                    ...
                    marker.on('dragend', function (e) {
                        ...
                        // 创建终点图标
                        const specialIcon = L.icon({
                            iconUrl: '../../../src/assets/imges/终点.png',
                            iconSize: [25, 25],
                            iconAnchor: [10, 18]
                        });
                        ...
                        // 创建红色实心圆图标（用于stop值为0的情况）
                        const redCircleIcon = L.icon({
                            iconUrl: '../../../src/assets/imges/圆圈.png',
                            iconSize: [12, 12],
                            iconAnchor: [6, 6]
                        });
                    ...
                    marker.on('click', function (e) {
                        ...
                        if (allowAddingStations && specialStation) {
                            ...
                            const tempStation = {
                                ...
                                stopTime: specialStation.stopTime != null ? specialStation.stopTime : "0",
            ...
            toggleStationVisibility(); // 确保特殊点标签的显示状态与按钮状态同步
        } else {
...
function renderStations() {
    ...
    stationData.forEach(station => {
        ...
        if (!existingMarker) {
            ...
            // 添加鼠标悬浮事件，控制标签显示
            marker.on('mouseover', function() {
                // 不再需要调用openTooltip，因为permanent: true会保持标签始终可见
                // 使用CSS类控制标签显示
                const tooltip = this.getTooltip();
                if (tooltip) {
                    L.DomUtil.addClass(tooltip._container, 'leaflet-tooltip-visible');
        ...
        if (selectedRouteId) {
            ...
            const result = routePoints.map((point, index) => {
                ...
                return {
                    ...
                    area: tempStation ? tempStation.area : (originalStation ? originalStation.area : (localStorage.getItem('globalObstacleLevel') || "1")),
                    direction: directionValue,
                    stopTime: tempStation && tempStation.stopTime != null
                        ? tempStation.stopTime
                        : (originalStation && originalStation.stopTime != null ? originalStation.stopTime : "0"),
                    position: tempStation ? tempStation.position : (originalStation ? originalStation.position : (localStorage.getItem('globalNavigationMode') || "1")),
                    stop: tempStation ? tempStation.stop : (originalStation ? originalStation.stop : "0"),
                    runmode: tempStation ? tempStation.runmode : (originalStation ? originalStation.runmode : (localStorage.getItem('globalTurnMode') || "0")),
                    lanechange: tempStation ? tempStation.lanechange : (originalStation ? originalStation.lanechange : (localStorage.getItem('globalLanechange') || "0")),
            ...
            try {
                console.log(updateMessage,"1updateMessage")
...
function addCuurrentPoint() {
    ...
    // 从positionData JSON对象中获取位置和姿态数据
    const positionDataStr = localStorage.getItem('positionData');
    let localX, localY, orientationX, orientationY, orientationZ, orientationW;
    
    if (positionDataStr) {
        try {
            const positionData = JSON.parse(positionDataStr);
            localX = positionData.localX;
            localY = positionData.localY;
            orientationX = positionData.orientationX;
            orientationY = positionData.orientationY;
            orientationZ = positionData.orientationZ;
            orientationW = positionData.orientationW;
        } catch (e) {
            console.error("解析positionData失败:", e);
            localX = null;
            localY = null;
...
window.calculateDirectionFromPoints = function(stationId, boxIndex, centerLat, centerLng) {
   ...
   if (previewLine) {
    ...
    window.centerMarker = L.marker([centerLat, centerLng], {
        icon: L.icon({
            iconUrl: '../../../src/assets/imges/中文起点.svg',
            iconSize: [20, 20],
            iconAnchor: [10, 10]
        })
    }).addTo(map);
    ...
    window.directionCalculateMapClickHandler = function(e) {
        ...
        window.directionTempMarker = L.marker([targetLat, targetLng], {
            icon: L.icon({
                iconUrl: '../../../src/assets/imges/中文终点.svg',
                iconSize: [20, 20],
                iconAnchor: [10, 10]
            })
        }).addTo(map);
