...
# 仓库源：deploy/start/ros2.sh；部署到 <部署根>/ros2.sh 后运行（脚本以自身位置为部署根）
# 加载页能力来自 ${BOOT_BRANDING:-$HOME/boot-branding}/boot-splash，缺失时自动降级为无加载页模式。
...
if ! ss -ltn 2>/dev/null | grep -q ':8088 '; then
  ...
  (
    cd "$APP_DIR"
    nohup bash "$APP_DIR/deploy/start/run-java.sh" \
