...
: Node("obstacle_detector_node", options)
{
  ...
  laser_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>("scan_se", sensor_qos, std::bind(&ObstacleDetector::laserCallback, this, _1));
...
void ObstacleDetector::laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr _msg)
{
    ...
    try
    {
      transform_stamped = tf_buffer_->lookupTransform("map", _msg->header.frame_id,  tf2::TimePointZero);
    }
    ...
    // 获取传感器在全局坐标系中的位置（用于设置地图原点）
    double sensor_x = transform_stamped.transform.translation.x;
    double sensor_y = transform_stamped.transform.translation.y;
    ...
    {
      x_min = sensor_x -5;//std::min(x_min, point.first);
      y_min = sensor_y -5;//std::min(y_min, point.second);
      x_max = sensor_x +5;//std::max(x_max, point.first);
      y_max = sensor_y +5;//std::max(y_max, point.second);
    }
    ...
    //RCLCPP_INFO(this->get_logger(), "laserCallback executed in %ld ms", duration.count());
}
