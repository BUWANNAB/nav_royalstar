...
namespace dwa_ros2
{
    ...
    DWA::DWA(const rclcpp::NodeOptions &options) : Node("dwa_ros2", options)
    {
        ...
        odometry_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("pcl_odom", 10, std::bind(&DWA::odomCallback, this, _1));
    ...
    void DWA::initParameter(void)
    {
        ...
        param_.max_omega = this->declare_parameter("max_omega", 2.0);
        ...
        param_.v_resolution = this->declare_parameter("v_resolution", 0.02);
        ...
        param_.goal_cost_gain = this->declare_parameter("goal_cost_gain", 4.0);//0.3

        // Declare speed cost gain parameter
        param_.speed_cost_gain = this->declare_parameter("speed_cost_gain", 3.0);//1.5

        // Declare obstacle cost gain parameter0.2
        param_.obstacle_cost_gain = this->declare_parameter("obstacle_cost_gain", 2.5);//0.1

        // Declare robot radius parameter
        param_.robot_radius = this->declare_parameter("robot_radius", 0.3);

        param_.goal_tolerance = this->declare_parameter("goal_tolerance", 0.2);

        frame_id_ = this->declare_parameter("frame_id_", "map");
    }
    ...
    double DWA::calcObstacleCost(std::vector<State> trajectory)
    {
        ...
        double cost = (min_distance > param_.robot_radius) ? 1.0 / min_distance : 10000;//std::numeric_limits<double>::max();
        //RCLCPP_INFO(this->get_logger(), "Min Distance from Obstacle: %lf", min_distance);
        //RCLCPP_INFO(this->get_logger(), "Obstacle Cost: %lf", cost);
        return cost;
    }
    
    bool DWA::directionCalaulation(std::vector<State> trajectory)
    {
        double min_distance = std::numeric_limits<double>::max();
        Point min_trajectory; 
        Point min_obstacle; 
        for (auto &state : trajectory)
        {
            for (auto &obstacle : obstacles_)
            {
                ...
                if (distance < min_distance) 
                {
                    ...
                    min_trajectory.x = state.x;
                    min_trajectory.y = state.y;
                    min_obstacle.x = obstacle.first;
                    min_obstacle.y = obstacle.second;
                }
            }
        }
        double direction = min_trajectory.y - min_obstacle.y;
    ...
    void DWA::dwaControl()
    {
        // Update Parameter
        updateParameter();
        //RCLCPP_INFO(this->get_logger(), "11111111");
        ...
        /*
        double obstacle_cost2 = calcObstacleCost(best_trajectory);
        // 如果所有轨迹都被否决（best_cost仍为无穷大）
        if (obstacle_cost2 == 10000) {
            // 执行恢复行为：原地旋转寻找可行路径
            bool direction = directionCalaulation(best_trajectory);
            double recovery_omega = 0.5;  // 旋转角速度
            if (direction ==false) recovery_omega = -1*0.5;
            publishTwist(0.05, recovery_omega);
            RCLCPP_WARN(this->get_logger(), "No feasible path found, executing recovery behavior");
            return;
        }
        */
        //RCLCPP_INFO(this->get_logger(), "222222");
    ...
    std::vector<double> DWA::calcDynamicWindow(void)
    {
        ...
        double v_min = 0.1;//std::max(min_v, v - max_a * param_.dt);
        double v_max = 0.1;//std::min(max_v, v + max_a * param_.dt);
...
/*
        // 计算动态窗口时考虑更宽的范围
        double v_min = std::max(min_v, v - max_a * param_.dt - safety_margin);
        double v_max = std::min(max_v, v + max_a * param_.dt + safety_margin);
        double omega_min = std::max(min_omega, omega - max_a_omega * param_.dt - safety_margin);
        double omega_max = std::min(max_omega, omega + max_a_omega * param_.dt + safety_margin);
    
        // 如果当前速度接近0，允许更大的转向范围
        if (std::abs(v) < 0.05) {
            omega_min = -param_.max_omega;
            omega_max = param_.max_omega;
        }
*/        
