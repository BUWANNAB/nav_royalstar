...
def generate_launch_description():
    ...
    lidar_tf = launch_ros.actions.Node(
        ...
        arguments=['-0.5','0','0','0','0','0.7071','0.7071','base_link','livox_frame']
        )
        
    xslidar_tf = launch_ros.actions.Node(
        name='xslidar_tf',
        package='tf2_ros',
        executable='static_transform_publisher',
        arguments=['0','0','0','0','0','0','1','base_link','laser_se']
        )
    ...
    # 初始地图必须显式提供（BLUEANT_MAP_PCD）；默认留空并告警，等待运行时 /map_path 注入
    map_path = os.environ.get('BLUEANT_MAP_PCD', '')
    if not map_path:
        print('[lidar_localization.launch] WARN: BLUEANT_MAP_PCD 未设置，等待运行时 /map_path 或显式 launch 参数')
