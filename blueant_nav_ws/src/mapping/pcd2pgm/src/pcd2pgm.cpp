...
namespace pcd2pgm
{
// 展开 $ROBOT_APP_DIR（部署根，可任意路径；缺省回退 $HOME）/$HOME/开头的 ~；与 Web 端 ROBOT_PCD_DIR/ROBOT_MAP_DIR 契约一致
std::string expandHome(const std::string& path)
{
  const char* home = std::getenv("HOME");
  const char* app_dir = std::getenv("ROBOT_APP_DIR");
  const std::string root =
    (app_dir != nullptr && app_dir[0] != '\0') ? app_dir :
    (home != nullptr ? home : "");

  std::string result = path;
  if (!root.empty() && result.rfind("$ROBOT_APP_DIR", 0) == 0) {
    result.replace(0, 14, root);
  } else if (home != nullptr && result.rfind("$HOME", 0) == 0) {
    result.replace(0, 5, home);
  } else if (home != nullptr && result.rfind("~/", 0) == 0) {
...
void Pcd2PgmNode::loadDefaultParameters()
{
  ...
  // 展开部署根（$ROBOT_APP_DIR，缺省回退 $HOME），与 Web 端 ROBOT_PCD_DIR/ROBOT_MAP_DIR 契约一致
...
{
  declare_parameter("pcd_file", "$ROBOT_APP_DIR/pcd/point_cloud.pcd");
  ...
  declare_parameter("map_save_path", "$ROBOT_APP_DIR/maps/");  // 栅格地图保存路径
  declare_parameter("map_base_dir", "$ROBOT_APP_DIR/maps/");  // Web 与转换节点共用的地图根目录
  declare_parameter("map_save_name", "map");  // 栅格地图保存名称
  declare_parameter("rotated_pcd_save_path", "$ROBOT_APP_DIR/pcd/pcdmap.pcd");  // 旋转后的PCD文件保存路径
}
