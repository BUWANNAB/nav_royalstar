...
namespace pcd2pgm
{
// 展开路径中的 $HOME 与开头的 ~，与 Web 端 ${user.home}/inHome/... 保持一致
...
void Pcd2PgmNode::loadDefaultParameters()
{
  ...
  map_base_dir_ = DefaultParameters::DEFAULT_MAP_SAVE_PATH;
  ...
  // 展开 $HOME，与 Web 端 ${user.home}/inHome/... 保持一致
  ...
  config_file_path_ = expandHome(config_file_path_);
...
void Pcd2PgmNode::mapPathCallback(const std_msgs::msg::String::SharedPtr msg)
{
    ...
    // 启动文件提供配置路径时才持久化，避免绑定某个工作空间源码目录。
    if (!config_file_path_.empty()) {
      try {
          YAML::Node config = YAML::LoadFile(config_file_path_);
          config["pcd2pgm"]["ros__parameters"]["pcd_file"] = msg->data;
          config["pcd2pgm"]["ros__parameters"]["rotated_pcd_save_path"] = msg->data;
          std::ofstream fout(config_file_path_);
          fout << config;
          fout.close();
          RCLCPP_INFO(rclcpp::get_logger("pcd2pgm"),
                      "已更新YAML配置文件: %s", config_file_path_.c_str());
      } catch (const std::exception& e) {
          RCLCPP_ERROR(rclcpp::get_logger("pcd2pgm"),
                      "写入YAML配置文件失败: %s", e.what());
      }
    }
...
void Pcd2PgmNode::mapFolderCallback(const std_msgs::msg::String::SharedPtr msg)
{
    ...
    // 与 Web 端 pgm.root.path=${user.home}/inHome/maps/ 保持一致
    map_save_path_ =
...
{
  declare_parameter("pcd_file", "$HOME/inHome/pcd/point_cloud.pcd");
  ...
  declare_parameter("map_save_path", "$HOME/inHome/maps/");  // 栅格地图保存路径
  declare_parameter("map_base_dir", "$HOME/inHome/maps/");  // Web 与转换节点共用的地图根目录
  declare_parameter("map_save_name", "map");  // 栅格地图保存名称
  declare_parameter("rotated_pcd_save_path", "$HOME/inHome/pcd/pcdmap.pcd");  // 旋转后的PCD文件保存路径
  declare_parameter("config_file_path", "");
}
...
void Pcd2PgmNode::getParameters()
{
  ...
  get_parameter("config_file_path", config_file_path_);
