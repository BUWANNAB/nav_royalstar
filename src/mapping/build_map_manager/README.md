...
默认从当前用户主目录推导工作空间和地图目录，也可以通过环境变量覆盖：

```bash
export LIO_SAM_WS="$HOME/lio_sam_test"
export ROBOT_PCD_DIR="$HOME/inHome/pcd"
ros2 launch build_map_manager build_map_manager.launch.py
```
...
```bash
ros2 launch build_map_manager build_map_manager.launch.py \
  workspace_setup:="$HOME/lio_sam_test/install/setup.bash" \
  map_base_dir:="$HOME/inHome/pcd"
```
